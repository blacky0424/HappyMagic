...
public class CinderellaController : MonoBehaviour
{
    ...
    float m_shortInterval = 0.2f;
    float m_longInterval = 2.0f;
    float m_nexttime;
    //変身中
    bool m_isChange;
    ...
    void Start ()
    {
        ...
        IsFeintMode = false;
        m_nexttime = 0;
        m_commandButton = GameScene.Instance.CommandButton;
    ...
    private void Update()
    {
        ...
        //変装終了前の点滅
        if(m_fakeTimer > m_fakeLimit / 2)
        {
            if(Time.time > m_nexttime)
            {
                if (fakeMercenary.activeSelf)
                {
                    ChangeModel(fakeMercenary, cinderella);
                    m_nexttime += m_shortInterval;
                }
                else if (cinderella.activeSelf)
                {
                    ChangeModel(cinderella, fakeMercenary);
                    m_nexttime += m_longInterval;
                    m_longInterval *= 0.7f;
                }
    ...
    public void Restart()
    {
        ...
        this.tag = TagName.Player;
        m_commandButton.CountFadeReset();
        IsFeintMode = false;
        m_commandButton.FakeFadeReset();
        m_fakeMode = false;
        m_fakeTimer = 0;
